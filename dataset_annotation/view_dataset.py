...
class CocoDataset():
    ...
    def display_image(self, image_id, show_polys=True, show_bbox=True, show_crowds=True, use_url=False):
        ...
        rle_regions = {}
        poly_colors = {}
        if self.segmentations.get(image_id):
...
def view_dataset(annotation_path, image_dir):
    ...
    # view
    all_images = ""
    for img in coco_dataset.images.values():
        all_images += coco_dataset.display_image(img['id'], use_url=False)

    with open(image_dir + 'all_images.html', 'w') as outfile:
        outfile.write(all_images)